...
public class AutoLog extends Module {
    ...
    private final SettingGroup sgAntiChainPop = settings.createGroup("Chain Pop");
    ...
    private final Setting<Integer> health = sgGeneral.add(new IntSetting.Builder().name("health")
            ...
            .defaultValue(6).range(0, 19).sliderMax(19).build());

    private final Setting<Boolean> smart = sgGeneral.add(new BoolSetting.Builder().name("smart")
            .description(
    ...
    private final Setting<Boolean> instantDeath = sgGeneral.add(new BoolSetting.Builder()
            .name("32K").description("Disconnects when a player near you can instantly kill you.")
            .defaultValue(false).build());
    ...
    private final Setting<Set<EntityType<?>>> entities =
            sgEntities.add(new EntityTypeListSetting.Builder().name("entities").description(
                    "Disconnects when a specified entity is present within a specified range.")
                    .defaultValue(EntityType.END_CRYSTAL).build());
    ...
    private final Setting<Boolean> useTotalCount = sgEntities.add(new BoolSetting.Builder()
            ...
            .defaultValue(true).visible(() -> !entities.get().isEmpty()).build());
    ...
    private final Setting<Integer> combinedEntityThreshold = sgEntities.add(new IntSetting.Builder()
            ...
            .visible(() -> useTotalCount.get() && !entities.get().isEmpty()).build());

    private final Setting<Integer> individualEntityThreshold =
            sgEntities.add(new IntSetting.Builder().name("individual-entity-threshold").description(
                    "The minimum number of entities individually that must be near you before disconnection occurs.")
                    .defaultValue(2).min(1).sliderMax(16)
                    .visible(() -> !useTotalCount.get() && !entities.get().isEmpty()).build());

    private final Setting<Integer> range = sgEntities.add(new IntSetting.Builder().name("range")
            .description("How close an entity has to be to you before you disconnect.")
            .defaultValue(5).min(1).sliderMax(16).visible(() -> !entities.get().isEmpty()).build());

    // Anti chain pop
    private final Setting<Boolean> antiChainPop = sgAntiChainPop.add(new BoolSetting.Builder()
            .name("anti-chain-pop")
            .description(
                    "Toggle between counting the total number of all selected entities or each entity individually.")
            .defaultValue(true).build());
    ...
    public AutoLog() {
        super(Categories.Combat, "auto-log",
    ...
    private void onTick(TickEvent.Post event) {
        ...
        for (Long time : chainPops) {
            double difference = (currentTime - time) / 1000.0;
        ...
        if (popsWithinLastSeconds > chainPopLogCount.get()) {
            disconnect("Popped " + popsWithinLastSeconds + " totems within 1.5 seconds");
            return;
        }
        ...
        for (Entity entity : mc.world.getEntities()) {
            if (entity instanceof PlayerEntity player && player.getUuid() != mc.player.getUuid()) {
                if (onlyTrusted.get() && player != mc.player && !Friends.get().isFriend(player)) {
                    disconnect(Text.literal(
                            "Non-trusted player '" + Formatting.RED + player.getName().getString()
                                    + Formatting.WHITE + "' appeared in your render distance."));
                    if (toggleOff.get())
                        this.toggle();
                    return;
                }

                if (instantDeath.get() && PlayerUtils.isWithin(entity, 8)
                        && DamageUtils.getAttackDamage(player, mc.player) > playerHealth
        ...
        if (!entities.get().isEmpty()) {
            ...
            // Iterate through all entities in the world and count the ones that match the selected
            // types and are within range
            for (Entity entity : mc.world.getEntities()) {
                if (PlayerUtils.isWithin(entity, range.get())
                        && entities.get().contains(entity.getType())) {
                    totalEntities++;
                    if (!useTotalCount.get()) {
                        entityCounts.put(entity.getType(),
                                entityCounts.getOrDefault(entity.getType(), 0) + 1);
                    }
                }
            }
    ...
    private void onReceivePacket(PacketEvent.Receive event) {
        ...
        chainPops.add(System.currentTimeMillis());
    }
    ...
    private class StaticListener {
        ...
        private void healthListener(TickEvent.Post event) {
            ...
            else if (Utils.canUpdate() && !mc.player.isDead()
